...
\begin{frame}{$Q_{1}:$ Does \unistore{} Combine Causal and Strong Consistency Effectively?}
  \begin{center}
    \uncover<2->{\blue{Throughput:} $72\%$ and $183\%$ higher than \textsc{RedBlue} and \textsc{Strong}, \\[3pt]
    ...
    \vspace{0.50cm}
    \uncover<3->{\blue{Average latency:} $16.5$ms of \unistore{} \emph{vs.} $80.4$ms of \textsc{Strong}, \\[3pt]
...
\begin{frame}{$Q_{2}:$ How does \unistore{} Scale with the Number of Machines?}
  \begin{center}
    \uncover<2->{Scales almost \blue{linearly} (top): \\[3pt]
    ...
    \vspace{0.30cm}
    \uncover<3->{Under contention (bottom): $17.15\%$ throughput drop}
  \end{center}
\end{frame}
%%%%%%%%%%%%%%%%%%%%
