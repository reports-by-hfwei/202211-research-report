...
\begin{frame}{}
  \begin{center}
    ...
    \vspace{1.20cm}
    \red{关键挑战 (一):} 在容错的情况下保证系统的活性 (liveness) \\[10pt]
    \red{关键挑战 (二):} 严格的正确性证明
  \end{center}
\end{frame}
%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}{} % {\footnotesize{(\purple{ATC'2021})}}}
  \begin{center}
    ...
    \vspace{1.20cm}
    负责\blue{严格的正确性证明}工作 \\ (完成 \textsf{arXiv} 完整版本中的20页证明) \\[12pt]
    在证明过程中发现协议中一处\red{较为严重的错误} \\ (同样存在于经典协议中)
  \end{center}
\end{frame}
%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}{}
  \begin{center}
    ...
    \begin{enumerate}[(I)]
      \centering
      \item 默认情况下, 使用事务因果一致性
      \item 允许用户定义事务之间的冲突关系, 并使用可串行化顺序执行
    \end{enumerate}
  \end{center}
\end{frame}
%%%%%%%%%%%%%%%%%%%%
